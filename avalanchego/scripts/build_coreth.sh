...
if [[ ! -d "$coreth_path" ]]; then
  go get "github.com/tenderly/net-flare/coreth@$coreth_version"
fi
...
cd "$coreth_path"
go build $build_args -modcacherw -ldflags "-X github.com/tenderly/net-flare/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
