...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get -modcacherw "github.com/tenderly/net-flare/coreth@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -modcacherw -ldflags "-X github.com/tenderly/net-flare/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
